#include <kinect_pcl_obj_detection/pcl_fpfh_descriptors_matching.hpp>
...
void callback(const PCMsg::ConstPtr& kinect_pc_msg){
  ...
  // Remove NaNs
  vector<int> index;
  pcl::removeNaNFromPointCloud<PointType>(*cluster_cloud_,*cluster_cloud_,index);
  ...
  pcl::removeNaNNormalsFromPointCloud<NormalType>(*normals_,*normals_,index);
  ...
  bool use_hough_ (false), show_correspondences_(true), show_keypoints_(true);
...
//   descr_est.setSearchMethod(tree);
  ...
  pcl::FPFHEstimationOMP<PointType, NormalType, DescriptorType> descr_est2;
  descr_est2.setInputCloud(scene_keypoints);
  descr_est2.setInputNormals(normals_);
//   descr_est2.setSearchMethod(tree);
  descr_est2.setSearchSurface (cluster_cloud_);
  descr_est2.setRadiusSearch(0.05);
...
//     if (!pcl_isfinite (scene_descriptors->at(i).histogram)) //skipping NaNs 
//     { 
//       continue; 
//     } 
    ...
    { 
      if(found_neighs == 1 && neigh_sqr_dists[k] < 100.0f) //  add match only if the squared descriptor distance is less than 0.25 (SHOT descriptor distances are between 0 and 1 by design) 
      { 
